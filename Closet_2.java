...
public class Closet_2 {
    ...
    // 분할
    private void ClosestPair(int[][] A,int[][] D, int p, int q) { // 초기 값 p -> l (0 )  , q -> r ( S.length -1 : 9 )
        int i= q-p;
        if(i<=3) return; // 재귀 멈춤 조건
        int k = (p+q)/2; // 중간 index
        ClosestPair(A,D, p, k); // [10][2]로 가정했을 때, 초기 ( A, 0, 4 ) - > CP_L ( 왼쪽 재귀 호출)
        ClosestPair(A,D,k+1, q); // 초기 ( A, 5, 9 ) -> CP_R ( 오른쪽 재귀 호출
        merge(A,D, p, k, q);
    }
